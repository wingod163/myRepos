...
#######################################
# title:  各项营销活动拉新促活效果分析数据需求累计 
# author: zhaojl_bds
# date:   2018-09-26
# note:   总表出数，目标新用户采用旧分区，目标老用户采用新分区，因为跨月将分区进行衔接判断时间。
#######################################
...
#首先，将文件清空。
> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt

echo "#######################${v_thisyyyymmdd}各项营销活动拉新促活效果分析#######################" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "开始时间：startTime = "$startTime >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "系统时间：systime = "$systime >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "参数年月日：v_thisyyyymmdd = "${v_thisyyyymmdd} >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "参数年月：v_thisyyyymm = "${v_thisyyyymm} >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "参数年  ：v_thisyyyy = "${v_thisyyyy} >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "参数月  ：v_thismm = "${v_thismm} >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "参数日  ：v_thisdd = "${v_thisdd} >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt


#*********************************************************************************************************************************
#1.1.参与活动的客户情况-参与活动用户数
...
from (select t.bind_no
      ...
      where ((yearstr=2018 and monthstr=09 and daystr>=21) or (yearstr=${v_thisyyyy} and monthstr=${v_thismm} and daystr<=${v_thisdd}))
      and t.batch_id in ('1036535292015804416','1036539413569601536')
      and (regexp_replace(substr(t.bind_time,1,10),'-','')>='20180921'
...
#执行
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "1.1.参与活动的客户情况-参与活动用户数" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt



#*********************************************************************************************************************************
#1.2.参与活动的客户情况-目标新用户数
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "1.2.参与活动的客户情况-目标新用户数" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "1.3.参与活动的客户情况-目标老用户数" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
...
from (select a1.mb
      from (select wt_mobile as mb
            ...
            where (deal_date>='20180921'
            and deal_date<='${v_thisyyyymmdd}')
            and busi_type='wt'
      ...
      group by a1.mb) b1
        inner join (select serial_number
                   ...
                   where ((year=2018 and month=09 and day>=21) or (year=${v_thisyyyy} and month=${v_thismm} and day<=${v_thisdd}))
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "2.1.活跃情况-目标新用户登录APP" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "2.2.活跃情况-目标老用户登录APP" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
...
from (select a1.mb
      from (select t.bind_no as mb
            ...
            where ((yearstr=2018 and monthstr=09 and daystr>=21) or (yearstr=${v_thisyyyy} and monthstr=${v_thismm} and daystr<=${v_thisdd}))
            and t.batch_id in ('1036535292015804416','1036539413569601536')
            and (regexp_replace(substr(t.bind_time,1,10),'-','')>='20180921' and regexp_replace(substr(t.bind_time,1,10),'-','')<='${v_thisyyyymmdd}')
            and t.if_test='1' 
              group by t.bind_no) a1
                  inner join (select serial_number as mb
                              from pmrt.app_logins
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "2.3.活跃情况-沉默用户数" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "2.4.活跃情况-沉默用户数活跃数" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
...
echo "${sql}"
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "2.5.活跃情况-拉新促活" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
hive -e "${sql};" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt


#*********************************************************************************************************************************
echo "***********************************************" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
echo "script successful!" >> /e3base/zjl/result/tmrt_older2newer_day_sum_${v_thisyyyymmdd}.txt
